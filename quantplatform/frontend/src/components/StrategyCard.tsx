import type { Strategy, Metric } from "@/types";
...
export function StrategyCard({ strategy }: StrategyCardProps) {
  const metrics = strategy.metrics || defaultMetrics;
  const hasBacktest = !!strategy.metrics;
      ...
      <CardContent className="p-5 grid grid-cols-2 gap-4">
        {!hasBacktest ? (
          <div className="col-span-2 text-center py-4 text-muted-foreground">
            ...
            <p className="text-xs mt-1">Click to run backtest</p>
          </div>
        ) : (
          <>
            <div>
              ...
              <div className={`text-2xl font-bold font-mono tracking-tighter ${isPositive ? 'text-success' : 'text-destructive'}`}>
                {isPositive ? '+' : ''}{(metrics.total_return * 100).toFixed(1)}%
              </div>
            </div>
            <div>
